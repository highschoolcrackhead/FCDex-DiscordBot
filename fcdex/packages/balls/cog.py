...
class Balls(commands.GroupCog, group_name=settings.players_group_cog_name):
    ...
    async def completion(
        ...
        source.embed.colour = discord.colour.parse_hex_number("ffa45b")
    ...
    async def collection(
        ...
        embed = discord.Embed(
            ...
            description=desc,
            color=discord.colour.parse_hex_number("ffa45b"),
        )
        embed.set_author(
    ...
    async def rarity(
        ...
        """
        Show the rarity tiers of the clubs
        """
        ...
        entries = []
        j = 1
        for i in range(len(sorted_rarities)):
            collectible_names = "\n".join(
                [
                    f"\u200b • {self.bot.get_emoji(c.emoji_id) or 'N/A'} {c.country}"
                    for c in rarity_to_collectibles[sorted_rarities[i]]
                ]
            )
            entry = (f"▶ Tier {j}", f"{collectible_names}")
            entries.append(entry)
            j += len(rarity_to_collectibles[sorted_rarities[i]])
    ...
    async def leaderboard(
        ...
        """
        Display the player leaderboard based on the number of clubballs

        Parameters
        ----------
        weighted: bool
            Gives more value to rarer and special clubballs
        """
        await interaction.response.defer()
        ...
        # Calculate ball count for each player, filter blacklist
        if not weighted:
            player_stats = []
            for player in players:
                if player.discord_id in self.bot.blacklist:
                    continue  # Skip blacklisted players
                ball_count = len(player.balls)
                if ball_count > 0:  # Only show players with balls
                    player_stats.append((player, ball_count))
            ...
            for player in players:
                ...
                if player.discord_id in self.bot.blacklist:
                    continue
                if player.discord_id in [1165507477975093308, '1165507477975093308']:
                    continue
                if len(player.balls) > 0:
                    for ball in player.balls:
                        real_ball = await ball.ball
                        rarity = real_ball.rarity
                        if ball.special:
                            spec = await ball.special
                            spec_rarity = spec.rarity
                            points += (1//rarity)*(1//spec_rarity)
                        else:
                            points += (1//rarity)
                    player_stats.append((player, points))

            player_stats.sort(key=lambda x: x[1], reverse=True)


        if not player_stats:
            await interaction.followup.send("No players have collected any clubballs yet!")
            return

        # Create leaderboard message
        embed = discord.Embed(
            title="🏆 FC Dex Leaderboard",
            color=discord.colour.parse_hex_number("ffa45b")
        )

        # Display top 10 players, add medals for top 3
        medals = ["🥇", "🥈", "🥉"]
        lines = []
        for i, (player, count) in enumerate(player_stats[:10], 1):
            try:
            ...
            if not weighted:
                if i <= 3:
                    lines.append(f"{medals[i-1]} **{name}** - {count} clubballs")
                else:
                    lines.append(f"{i}. **{name}** - {count} clubballs")
            else:
                if i <= 3:
                    lines.append(f"{medals[i-1]} **{name}** - {count:.0f} points")
                else:
                    lines.append(f"{i}. **{name}** - {count:.0f} points")
        embed.description = "\n".join(lines)
