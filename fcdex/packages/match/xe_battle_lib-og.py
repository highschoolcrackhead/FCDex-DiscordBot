...
def attack(current_ball, enemy_balls):
    alive_balls = [ball for ball in enemy_balls if not ball.defeated]
    enemy = random.choice(alive_balls)

    attack_dealt = get_damage(current_ball)
    enemy.defense -= attack_dealt

    if enemy.defense <= 0:
        enemy.defense = 0
        enemy.defeated = True
    if enemy.defeated:
        gen_text = f"{current_ball.owner}'s {current_ball.name} has defeated {enemy.owner}'s {enemy.name}"
    else:
...
def gen_battle(battle: BattleInstance):
    ...
    while any(ball for ball in battle.p1_balls if not ball.defeated) and any(
        ball for ball in battle.p2_balls if not ball.defeated
    ):
        ...
        for p1_ball, p2_ball in zip(alive_p1_balls, alive_p2_balls):
            ...
            if not p2_ball.defeated:
                ...
                if all(ball.defeated for ball in battle.p1_balls):
                    break
    # Determine the winner

    if all(ball.defeated for ball in battle.p1_balls):
        battle.winner = battle.p2_balls[0].owner
    elif all(ball.defeated for ball in battle.p2_balls):
