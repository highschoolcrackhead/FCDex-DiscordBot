...
def gen_deck(balls) -> str:
    ...
    deck = "\n".join(
        [
            f"- {ball.emoji} {ball.name} (ATK: {ball.attack} | DEF: {ball.defense})"
            for ball in balls
        ]
    )
    if len(deck) > 1024:
...
class Match(commands.GroupCog):
    ...
    bulk = app_commands.Group(
        name='bulk', description='Bulk commands for match'
    )
    ...
    async def start_battle(self, interaction: discord.Interaction):
        ...
        if guild_battle.author_ready and guild_battle.opponent_ready:
            ...
            log_embed = discord.Embed(
                title=f"Match Recap ({guild_battle.author.name} vs {guild_battle.opponent.name})",
                description=battle_log,
                color=discord.colour.parse_hex_number("ffa45b"),
            )
            if guild_battle.battle.winner == "Draw":
                log_embed.set_footer(text=f"The match was a draw. Score: {guild_battle.battle.p1_goals}-{guild_battle.battle.p2_goals}")
            else:
                log_embed.set_footer(text=f"{guild_battle.battle.winner} won! Score: {guild_battle.battle.p1_goals}-{guild_battle.battle.p2_goals}")
            ...
            embed.add_field(
                name="Winner:",
                value=f"{guild_battle.battle.winner}",
                inline=False,
            )
            embed.add_field(
                name="Score:",
                value=f"{guild_battle.battle.p1_goals}-{guild_battle.battle.p2_goals}",
                inline=False
            )
    ...
    async def start(self, interaction: discord.Interaction, opponent: discord.Member):
        ...
        if fetch_battle(opponent) is not None:
            await interaction.response.send_message(
                "That user is already in a match.", ephemeral=True,
            )
            return
    ...
    async def remove_balls(self, interaction: discord.Interaction, countryballs):
        ...
        if (interaction.user == guild_battle.author and guild_battle.author_ready) or (
            ...
            await interaction.response.send_message(
                "You cannot change your clubballs as you are already ready.", ephemeral=True
            )
            return
        # Determine if the user is the author or opponent and get the appropriate ball list
    ...
    async def add(
        ...
        async for dupe in self.add_balls(interaction, [countryball]):
            if dupe:
                await interaction.response.send_message(
                    "You cannot add clubball twice!", ephemeral=True
                )
                return
        ...
        await interaction.response.send_message(
            f"Added `#{countryball.id} {countryball.countryball.country} ({health}%/{attack}%)`!",
            ephemeral=True,
        )
    ...
    async def remove(
        ...
        """
        Removes a clubball from battle.
        ...
        await interaction.response.send_message(
            f"Removed `#{countryball.id} {countryball.countryball.country} ({health}%/{attack}%)`!",
            ephemeral=True,
        )
