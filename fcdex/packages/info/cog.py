...
class Info(commands.Cog):
    ...
    async def about(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        """
        embed = discord.Embed(
            title=f"{settings.bot_name} Discord bot", color=discord.colour.parse_hex_number("ffa45b")
        )
        ...
        owner_credits = "by" if bot_info.team else "by"
        dex_credits = (
            f"This instance is owned {owner_credits} {owner}.\nAn instance of [Ballsdex]"
            f"({settings.github_link}) by El Laggron"
        )
        embed.description = (
            ...
            f"{dex_credits}\n\n"
            f"Consider supporting **FC Dex** on **[Patreon](https://patreon.com/fcdex)**\n\n"
            f"[Discord server]({settings.discord_invite}) • [Invite me]({invite_link}) • "
            f"[Patreon](https://patron.com/fcdex) • [Webpage](https://fcdex.vercel.app)\n"
            f"[Terms of Service]({settings.terms_of_service}) • [Source Code & Issues]({settings.github_link}) • "
    ...
    async def help(self, interaction: discord.Interaction["BallsDexBot"]):
        ...
        assert self.bot.user
        embed = discord.Embed(
            title=f"{settings.bot_name} Discord bot - help menu", color=discord.colour.parse_hex_number("ffa45b")
        )
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
